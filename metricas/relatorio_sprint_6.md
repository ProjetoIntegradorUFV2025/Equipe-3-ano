# 📊 Relatório de Métricas - Sprint 6

**Período:** 2025-10-27 até 2025-11-04  
**Data de geração:** 03/11/2025 20:59

## 🎯 MÉTRICAS INDIVIDUAIS SOLICITADAS

### 📊 1.3 - Produtividade de PRs
| Métrica | Valor |
|---------|-------|
| **PRs Criadas** | 36 |
| **PRs Aceitas** | 16 |
| **1.3 - Taxa Produtividade PRs** | **44.44%** |

### 👀 1.4 - PRs Revisadas
| Métrica | Valor |
|---------|-------|
| **1.4 - Total PRs Revisadas** | **29** |

### ✅ 1.5 - Taxa Aceitação Issues
| Métrica | Valor |
|---------|-------|
| **Issues Identificadas** | 2 |
| **Issues Aceitas** | 1 |
| **1.5 - Taxa Aceitação Issues** | **50.0%** |

## 🏢 MÉTRICAS DE EQUIPE SOLICITADAS

### ✅ 2.5 - Taxa Resolução Issues
| Métrica | Valor |
|---------|-------|
| **Issues Aceitos** | 1 |
| **Issues Resolvidos** | 1 |
| **2.5 - Taxa Resolução Issues** | **100.0%** |

### ⏱️ 2.7 - Tempo Médio Resolução Issues
| Métrica | Valor |
|---------|-------|
| **Tempo Total Resolução** | 104.04h |
| **Issues Resolvidos** | 1 |
| **2.7 - Tempo Médio Resolução** | **104.04h** |

### ⏱️ 2.9 - Tempo Médio Aprovação PRs
| Métrica | Valor |
|---------|-------|
| **Tempo Total Aprovação** | 317.58h |
| **PRs Aprovadas** | 16 |
| **2.9 - Tempo Médio Aprovação** | **19.85h** |

## 🔄 MÉTRICAS DE QUALIDADE


### 💬 Code Review
| Métrica | Valor |
|---------|-------|
| **Total Comentários** | 29 |
| **PRs Analisados** | 36 |
| **Nº Médio Comentários/Revisão** | **0.81** |

## 👥 MÉTRICAS INDIVIDUAIS DETALHADAS

| Usuário | 1.3 PRs Criadas | 1.3 PRs Aceitas | 1.3 Produtividade | 1.4 PRs Revisadas | 1.5 Issues Identificadas | 1.5 Issues Aceitas | 1.5 Taxa Aceitação |
|---------|----------------|----------------|------------------|------------------|-------------------------|-------------------|-------------------|
| Tais0357 | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| Gabriwll | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| Eufrausino | 1 | 1 | 100.0% | 3 | 2 | 1 | 50.0% |
| RayannePapini | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| Tatiana0357 | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| GuilhermePianetti | 3 | 3 | 100.0% | 4 | 0 | 0 | 0% |
| gabriel2036 | 2 | 2 | 100.0% | 6 | 0 | 0 | 0% |
| ingridlrs | 0 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| GuilhermeGC05 | 1 | 1 | 100.0% | 0 | 0 | 0 | 0% |
| ProjetoIntegradorUFV2025 | 0 | 0 | 0% | 0 | 0 | 0 | 0% |
| francyelleserrano | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| DalmoRainer | 0 | 0 | 0.0% | 0 | 0 | 0 | 0% |
| Analucia26 | 1 | 1 | 100.0% | 2 | 0 | 0 | 0% |
| amandacmelo | 0 | 0 | 0% | 0 | 0 | 0 | 0% |
| melissaalanis | 0 | 0 | 0% | 0 | 0 | 0 | 0% |
| cristinaleticia | 0 | 0 | 0% | 0 | 0 | 0 | 0% |
| caioomenezes | 1 | 1 | 100.0% | 0 | 0 | 0 | 0% |
| DaniiMelo | 1 | 0 | 0.0% | 0 | 0 | 0 | 0% |


## 📋 Legenda das Métricas

### 🎯 Métricas Individuais
- **1.3 - Produtividade PRs**: Nº de PRs aceitas / Nº de PRs criadas pelo aluno
- **1.4 - PRs Revisadas**: Nº de PRs revisadas pelo aluno  
- **1.5 - Taxa Aceitação Issues**: Issues aceitos do aluno / Issues identificados pelo aluno

### 🏢 Métricas de Equipe
- **2.5 - Taxa Resolução Issues**: Issues resolvidos / Issues Aceitos
- **2.7 - Tempo Médio Resolução**: Tempo total resolução / Issues resolvidos
- **2.9 - Tempo Médio Aprovação**: Tempo total revisão / PRs aprovadas

### 🔄 Qualidade
- **Taxa de Retrabalho**: Commits rejeitados / Commits submetidos × 100
- **Nº Médio Comentários/Revisão**: Total comentários / Total PRs analisados
